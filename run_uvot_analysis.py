...
class uvot_runner(object):
    ...
    def uvot_checker(self):
        ...
        for filepath in self.filepaths:
            ...
            iv.update()  # x = input('Viewing %s. Hit Enter to continue.' %filepath)
    ...
    def uvot_measurer(self, measure=True, default_fs=True):
        ...
        ptab = Table(names=(
        'filter', 'MJD', 'Mag', 'MagErr', 'FluxDensity', 'FluxDensityErr', 'FluxDensityJy', 'FluxDensityJyErr',
        'FluxExtCorr', 'FluxExtCorrErr'), dtype=("S", float, float, float, float, float, float, float, float, float))
...
def main():
    parser = argparse.ArgumentParser(
        description='Quick verification of image quality and regions selection for UVOT analysis.')
    parser.add_argument('-p', required=True,
                        help='Parent directory of UVOT data structures (directories with names like 00030901252).')
    parser.add_argument('-obs', default=None,
                        help='If specified, will only look at data for a single observations: e.g. 00030901252')
    parser.add_argument('-f', default=None,
                        help='File with list of (full or relative) directory paths of UVOT data structures. Use this option if UVOT data structure directory names do not start with 000 for some reason...')
    parser.add_argument('-o', default='photometry.fits',
                        help='Name of output fits file for storing resulting photometry.')
    parser.add_argument('-c', default=None, help='Specify coordinates for the source of interest.')
    parser.add_argument('-s', default=None,
                        help='Specify the name for the source of interest. Ignored if -c is specified.')
    parser.add_argument('-ebv', default=None,
                        help='User-specified E(B-V) value. Will query IRSA database, if not specified.')
    parser.add_argument('-date_range', default='1990-01-01,2020-01-01',
                        help='Comma-separated beginning and end of date(time) range to average images for SED construction. Format is somewhat flexible, but try \'YYYY-MM-DD HH:MM,YYYY-MM-DD HH:MM\'')
    parser.add_argument('--check', action='store_true', default=False,
                        help='View all images with the source marked to check observation quality.')
    ...
    parser.add_argument('--measure', action='store_true', default=False,
                        help='Run UVOTSOURCE on all images to get photometry.')
    parser.add_argument('--extract_only', action='store_true', default=False,
                        help='Extract photometry information from existing UVOTSOURCE output files, without rerunning UVOTSOURCE.')
    parser.add_argument('--sed', action='store_true', default=False,
                        help='Extract source SED by combining all observations/images located in the path (specified with -p) and falling within the date range (specified by -date_range)')
    parser.add_argument('--background', default=None,
                        help='Background to be used for the analysis. Expecting a .reg-type file.')
    ...
    if not (args.measure or args.check or args.detect or args.extract_only or args.sed):
        raise parser.error(
            'Nothing to do. Use --detect, --check, --measure, or --extract_only flags for desired operations or -h for help.')
    ...
    if args.c:
        ...
        prime_source = True
        print(
            'Source not specified by either -c or -s option. Will make user select the source location interactively.')
        print(
            'Quit (ctrl-C) and specifiy source name with -s or source coordinates with -c options if the source is known.')

    # Get the user defined background region
    if args.background:
        runner.bkg_region = Regions.read(args.background, format='ds9')[0] #take first item
    ...
    if args.sed:
        ...
        try:
            ...
            if start.lstrip().isdigit() and start.lstrip().isdigit():
                runner.uvot_sed_maker(Time(np.float(start.lstrip()), format='mjd'),
                                      Time(np.float(end.lstrip()), format='mjd'), outbase)
            else:
